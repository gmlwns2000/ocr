...
class COCO_Text:
    def __init__(self, annotation_file=None, ignoreCache=False):
        """
        ...
        self.test = []
        self.train = []
        self.cacheAnnsCatIds = None
        self.cacheAnnsCatIdsList = None
        if not annotation_file == None:
            assert os.path.isfile(annotation_file), "file does not exist"
            print('loading annotations into memory...')
            s = datetime.datetime.utcnow()
            if ignoreCache or (not os.path.exists('cocoText.cache')):
                time_t = datetime.datetime.utcnow()
                dataset = json.load(open(annotation_file, 'r'))
                print(datetime.datetime.utcnow() - time_t)
                self.dataset = dataset
                self.createIndex()
                print('load total', datetime.datetime.utcnow() - s)
                self.createCache()
            else:
                self.loadCache()
                print('load total', datetime.datetime.utcnow() - s)
    ...
    def loadCache(self):
        print('cache loading...')
        with open('cocoText.cache','rb') as f:
            state = cPickle.load(f)
        self.imgToAnns = state['imgToAnns']
        self.imgs = state['imgs']
        self.anns = state['anns']
    ...
    def createCache(self):
        ...
        with open('cocoText.cache','wb') as f:
    ...
    def getAnnIds(self, imgIds=[], catIds=[], areaRng=[], minSize=[]):
        """
        ...
        if len(imgIds) == len(catIds) == len(areaRng) == 0:
            ...
            if not len(imgIds) == 0:
                ...
                anns = list(self.anns.keys())
            if self.cacheAnnsCatIds != catIds:
                print("cached!")
                self.cacheAnnsCatIds = catIds
                self.cacheAnnsCatList = self.getAnnByCat(catIds)
            anns = anns if len(catIds)  == 0 else list(set(anns).intersection(set(self.cacheAnnsCatList))) 
            anns = anns if len(areaRng) == 0 else [ann for ann in anns \
                                                   if self.anns[ann]['area'] > areaRng[0] and self.anns[ann]['area'] < areaRng[1]]
            anns = anns if len(minSize) == 0 else [ann for ann in anns \
                                                   if self.anns[ann]['bbox'][2] > minSize[0] and \
                                                      self.anns[ann]['bbox'][3] > minSize[1]]
        return anns
